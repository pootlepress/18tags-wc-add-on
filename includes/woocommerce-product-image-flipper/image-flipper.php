	...
	if ( ! class_exists( 'WC_pif' ) ) {
		...
		class WC_pif {
			...
			function pif_scripts() {
				if ( ! apply_filters( 'eighteen-tags-wc-flip-prod-img', false ) ) { return; }
				if ( apply_filters( 'woocommerce_product_image_flipper_styles', true ) ) {
					wp_enqueue_style( 'pif-styles', plugin_dir_url( __FILE__ ) . '/assets/css/style.css' );
				}
				wp_enqueue_script( 'pif-script', plugin_dir_url( __FILE__ ) . '/assets/js/script.js', array( 'jquery' ) );
			}

			// Add pif-has-gallery class to products that have a gallery
			function product_has_gallery( $classes ) {
				if ( ! apply_filters( 'eighteen-tags-wc-flip-prod-img', false ) ) { return $classes; }
				global $product;
